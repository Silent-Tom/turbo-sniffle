  ...
  for (var round in json) {
    ...
    for (var enemy in json[round].enemy) {
      for (var part in json[round].enemy[enemy].children) {
		...
		var items = json[round].enemy[enemy].children[part].params[0].def_attributes;
		var filtered = items.filter(function(item) { 
		   return item.attribute_id != 202 && item.attribute_id != 203 && item.attribute_id != 206 && item.attribute_id != 213;
		});
		json[round].enemy[enemy].children[part].params[0].def_attributes = filtered;
        rows.push('Debuffed');
